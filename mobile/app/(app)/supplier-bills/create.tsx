...
import { getSuppliers, getSupplierBill, getSupplierBillItems, createSupplierBill, updateSupplierBill, getNextBillNumber, calcInvoiceTotals, LineItemDraft, fmtCurrency } from '@/lib/api';
...
export default function SupplierBillFormScreen() {
  ...
  useEffect(() => {
    if (isEdit) {
      (async () => {
        try {
          const [b, its] = await Promise.all([getSupplierBill(Number(id)), getSupplierBillItems(Number(id))]);
          if (!b) return;
          setBillNumber(b.bill_number ?? '');
          setSupplierId(b.supplier_id ?? null);
          setDate(b.date ?? '');
          setDueDate(b.due_date ?? '');
          setTrn(b.trn ?? '');
          setLpoNo(b.lpo_no ?? '');
          setNotes(b.notes ?? '');
          setAmountPaid(b.amount_paid ?? 0);
    ...
    } else { getNextBillNumber().then(setBillNumber).catch(() => {}); }
  }, [id]);
  ...
  const save = async () => {
    if (!billNumber.trim()) { Alert.alert('Required', 'Bill number is required.'); return; }
    ...
    try {
      const billData = {
        bill_number: billNumber.trim(), supplier_id: supplierId, date,
        due_date: dueDate || null, trn: trn || null, lpo_no: lpoNo || null, notes: notes || null,
        amount_paid: amountPaid,
      };
      if (isEdit) await updateSupplierBill(Number(id), billData, validItems);
      else await createSupplierBill(billData, validItems);
      router.back();
  ...
  return (
    <SafeAreaView style={s.safe}>
      <View style={s.navbar}>
        ...
        <Text style={s.navTitle}>{isEdit ? 'Edit Bill' : 'New Supplier Bill'}</Text>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={s.body} keyboardShouldPersistTaps="handled">
          <View style={s.card}>
            ...
            <Field label="Bill Number" value={billNumber} onChange={setBillNumber} placeholder="BILL-0001" />
            ...
            <Field label="Bill Date" value={date} onChange={setDate} placeholder="YYYY-MM-DD" keyboard="numeric" />
            <Field label="Due Date" value={dueDate} onChange={setDueDate} placeholder="YYYY-MM-DD (optional)" keyboard="numeric" />
            <Field label="Supplier TRN" value={trn} onChange={setTrn} placeholder="Optional" />
            <Field label="LPO / PO Reference" value={lpoNo} onChange={setLpoNo} placeholder="Optional" />
          </View>
          ...
          <View style={s.card}>
            ...
            <Field label="Notes" value={notes} onChange={setNotes} placeholder="Remarks…" multiline />
          </View>
