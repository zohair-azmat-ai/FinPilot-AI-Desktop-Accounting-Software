...
import { getPayments, getCustomers, deletePayment, fmtCurrency, fmtDate } from '@/lib/api';
...
export default function PaymentsScreen() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      setPayments((pays ?? []).filter((p: any) => p.payment_direction === 'received' || !p.payment_direction));
  ...
  const filtered = payments.filter(p =>
    ...
    (customers[p.customer_id] ?? '').toLowerCase().includes(search.toLowerCase()) ||
    ...
    Alert.alert('Delete', `Delete payment ${item.payment_number}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete', style: 'destructive', onPress: async () => {
          try { await deletePayment(item.id); load(); }
    ...
    <SafeAreaView style={s.safe}>
      ...
      {loading ? <View style={s.center}><ActivityIndicator color={C.brand} /></View> : (
        <FlatList data={filtered} keyExtractor={p => String(p.id)}
            ...
            <View style={s.card}>
              <View style={s.cardTop}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={s.sub}>{customers[item.customer_id] ?? '—'}</Text>
          ...
          ListEmptyComponent={<View style={s.center}><Ionicons name="cash-outline" size={44} color={C.border} /><Text style={s.emptyText}>No payments recorded</Text></View>}
          contentContainerStyle={{ paddingBottom: 80 }}
        />
      )}
      <TouchableOpacity style={s.fab} onPress={() => router.push('/(app)/payments/create')}>
