...
import { getDeliveryNotes, getCustomers, fmtDate } from '@/lib/api';
import { C } from '@/lib/theme';

const STATUS_COLOR: Record<string, string> = { draft: '#64748b', delivered: '#059669' };
const STATUS_BG: Record<string, string> = { draft: '#f1f5f9', delivered: '#d1fae5' };
...
export default function DeliveryNotesScreen() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const [dns, custs] = await Promise.all([getDeliveryNotes(), getCustomers()]);
      setNotes(dns ?? []);
  ...
  const filtered = notes.filter(n => !search || n.dn_number?.toLowerCase().includes(search.toLowerCase()) || (customers[n.customer_id] ?? '').toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <SafeAreaView style={s.safe}>
      <View style={s.header}>
        ...
        <Text style={s.title}>Delivery Notes</Text>
        <Text style={s.count}>{filtered.length}</Text>
      </View>
      <View style={s.searchWrap}>
        <Ionicons name="search-outline" size={16} color={C.muted} style={{ marginRight: 8 }} />
        <TextInput style={s.searchInput} placeholder="Search delivery notes..." placeholderTextColor={C.muted} value={search} onChangeText={setSearch} />
      </View>
      {loading ? <View style={s.center}><ActivityIndicator color={C.brand} /></View> : (
        <FlatList data={filtered} keyExtractor={n => String(n.id)}
          ...
          ListEmptyComponent={<View style={s.center}><Ionicons name="cube-outline" size={44} color={C.border} /><Text style={s.emptyText}>No delivery notes</Text></View>}
          contentContainerStyle={{ paddingBottom: 80 }}
        />
      )}
      <TouchableOpacity style={s.fab} onPress={() => router.push('/(app)/delivery-notes/create')}>
