...
_dbg(">>> ACTIVE PDF GENERATOR BUILD=FP_BLUE_V5_HF LOADED <<<")
...
# HF Space: assets live in /tmp/assets/ (uploaded via POST /api/assets/upload)
# Bundle assets directory (committed to HF Space repo) as secondary fallback
_HERE = os.path.dirname(os.path.abspath(__file__))
_TMP_LH  = "/tmp/assets/letterhead.jpg"
_REPO_LH = os.path.join(_HERE, "assets", "letterhead.jpg")
LETTERHEAD_PATH = _TMP_LH if os.path.exists(_TMP_LH) else _REPO_LH

_TMP_STAMP  = "/tmp/assets/stamp.png"
_REPO_STAMP = os.path.join(_HERE, "assets", "stamp.png")


def _get_stamp_path() -> str:
    for p in (_TMP_STAMP, _REPO_STAMP):
        if os.path.exists(p):
            return p
    return ""


def _refresh_asset_paths():
    """Re-evaluate which asset files exist (called before each PDF generation)."""
    global LETTERHEAD_PATH
    LETTERHEAD_PATH = _TMP_LH if os.path.exists(_TMP_LH) else _REPO_LH
